// Header file for chapter 15 - BLACK15.H
...
typedef struct wall_typ
{
	...
	vector_3d normal;        // the outward normal to the wall used during creation of BSP only, after that it becomes invalid
...
void Draw_TB_Tri_3D_Z(int x1, int y1, int z1, int x2, int y2, int z2, int x3, int y3, int z3, int color)
{
	...
	if (x1 >= poly_clip_min_x && x1 <= poly_clip_max_x && x2 >= poly_clip_min_x && x2 <= poly_clip_max_x && x3 >= poly_clip_min_x && x3 <= poly_clip_max_x) {
		// draw the triangle
...
void Draw_Tri_3D_Z(int x1, int y1, int z1, int x2, int y2, int z2, int x3, int y3, int z3, int color)
{
...
void Draw_Poly_List_Z(void)
{
	...
	for (curr_poly = 0; curr_poly < num_polys_frame; curr_poly++) {
		...
		// draw triangle
		Draw_Tri_3D_Z((int)x1, (int)y1, (int)z1, (int)x2, (int)y2, (int)z2, (int)x3, (int)y3, (int)z3, world_polys[curr_poly]->shade);
		...
		if (is_quad) {
			...
			// draw triangle
			Draw_Tri_3D_Z((int)x1, (int)y1, (int)z1, (int)x3, (int)y3, (int)z3, (int)x4, (int)y4, (int)z4, world_polys[curr_poly]->shade);
		}
	}
}
...
void Fill_Z_Buffer(int value)
{
	...
	for (index = 0; index < z_bank_size / sizeof(int); index++) {
		z_bank_1[index] = value;
		z_bank_2[index] = value;
	}
}
...
void Bsp_Traverse(wall_ptr root)
{
	...
	Make_Vector_3D((point_3d_ptr)&root->wall_world[0], (point_3d_ptr)&view_point, (vector_3d_ptr)&test_vector);

	// now dot test vector with the surface normal and analyze signs
	dot_wall = Dot_Product_3D((vector_3d_ptr)&test_vector, (vector_3d_ptr)&root->normal);
	...
	// SECTION 3 ////////////////////////////////////////////////////////////////
		...
		if ((z1 > clip_near_z && z1 < clip_far_z) || (z2 > clip_near_z && z2 < clip_far_z) ||
			...
			// now copy vertices, note that we don't use a structure copy, it's not dependable
			world_poly_storage[num_polys_frame].vertex_list[0].x = root->wall_camera[0].x;
			world_poly_storage[num_polys_frame].vertex_list[0].y = root->wall_camera[0].y;
			world_poly_storage[num_polys_frame].vertex_list[0].z = root->wall_camera[0].z;
...
void Bsp_Print(wall_ptr root)
{
	...
	fprintf(fp_out, "\n\n\nWall ID #%d", root->id);
	fprintf(fp_out, "\nVertices...");
	fprintf(fp_out, "\nVertex 0: (%f,%f,%f)", root->wall_world[0].x, root->wall_world[0].y, root->wall_world[0].z);
	fprintf(fp_out, "\nVertex 1: (%f,%f,%f)", root->wall_world[1].x, root->wall_world[1].y, root->wall_world[1].z);
	fprintf(fp_out, "\nVertex 2: (%f,%f,%f)", root->wall_world[2].x, root->wall_world[2].y, root->wall_world[2].z);
	fprintf(fp_out, "\nVertex 3: (%f,%f,%f)", root->wall_world[3].x, root->wall_world[3].y, root->wall_world[3].z);
	fprintf(fp_out, "\nNormal (%f,%f,%f)", root->normal.x, root->normal.y, root->normal.z);
	fprintf(fp_out, "\nEnd wall data\n");
...
void Build_Bsp_Tree(wall_ptr root)
{
	...
	static float
		...
		pp_x1, pp_y1, pp_z1,
		xi, zi;                     // points of intersection when the partioning plane cuts a wall in two

	static vector_3d test_vector_1,  // test vectors from the partioning plane
		test_vector_2;        // to the test wall to test the side of the partioning plane the test wall lies on
	...
	while (next_wall) {
		...
		// test wall
		Make_Vector_3D((point_3d_ptr)&root->wall_world[0], (point_3d_ptr)&next_wall->wall_world[0], (vector_3d_ptr)&test_vector_1);
		Make_Vector_3D((point_3d_ptr)&root->wall_world[0], (point_3d_ptr)&next_wall->wall_world[1], (vector_3d_ptr)&test_vector_2);

		// now dot each test vector with the surface normal and analyze signs
		dot_wall_1 = Dot_Product_3D((vector_3d_ptr)&test_vector_1, (vector_3d_ptr)&root->normal);
		dot_wall_2 = Dot_Product_3D((vector_3d_ptr)&test_vector_2, (vector_3d_ptr)&root->normal);
		...
		// SECTION 6  ////////////////////////////////////////////////////////////////
		...
		else if ((dot_wall_1 < 0 && dot_wall_2 >= 0) || (dot_wall_1 >= 0 && dot_wall_2 < 0)) {
			...
			Intersect_Lines(wall_x0, wall_z0, wall_x1, wall_z1, pp_x0, pp_z0, pp_x1, pp_z1, &xi, &zi);
