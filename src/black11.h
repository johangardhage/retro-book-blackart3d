// Header file for chapter 11 - BLACK11.H
...
{
	float x, y, z, w;    // a 3-D vector along with normalization factor if needed
...
typedef struct facet_typ
{
	...
	int active;      // used to turn faces on and off
	point_3d vertex_list[MAX_POINTS_PER_POLY]; // the points that make up the polygon facet
...
matrix_4x4 global_view;           // the global inverse wortld to camera matrix
...
facet world_poly_storage[MAX_POLYS_PER_FRAME];  // the storage for the visible polygons is pre-allocated so it doesn't need to be allocated frame by frame
...
void Draw_Triangle_2D_Text(int x1, int y1, int x2, int y2, int x3, int y3, unsigned char *buffer, int texture_index)
{
...
void Draw_Triangle_2D_Gouraud(int x1, int y1, int x2, int y2, int x3, int y3, unsigned char *buffer, int intensity_1, int intensity_2, int intensity_3)
{
...
void Triangle_Line(unsigned char *dest_addr, unsigned int xs, unsigned int xe, int color)
{
	...
	for (index = 0; index < xe - xs + 1; index++) {
		*dest_addr++ = color;
	}
}
...
void Draw_Top_Triangle(int x1, int y1, int x2, int y2, int x3, int y3, int color)
{
	...
	// compute starting address in video memory
	dest_addr = RETRO.framebuffer + (y1 << 8) + (y1 << 6);
...
void Draw_Triangle_2D(int x1, int y1, int x2, int y2, int x3, int y3, int color)
{
	int temp_x,
		temp_y,
		new_x;
...
void Local_To_World_Object(object_ptr the_object)
{
	...
	for (index = 0; index < the_object->num_vertices; index++) {
		the_object->vertices_world[index].x = the_object->vertices_local[index].x + the_object->world_pos.x;
		the_object->vertices_world[index].y = the_object->vertices_local[index].y + the_object->world_pos.y;
		the_object->vertices_world[index].z = the_object->vertices_local[index].z + the_object->world_pos.z;
	}
...
void Scale_Object(object_ptr the_object, float scale_factor)
{
	...
	float scale_2;    // holds the sqaure of the scaling factor, needed to resize the surface normal for lighting calculations
...
int PLG_Load_Object(object_ptr the_object, const char *filename, float scale)
{
	...
	for (index = 0; index < total_polys; index++) {
		...
		// the vector u = vo->v1
		Make_Vector_3D((point_3d_ptr)&the_object->vertices_local[vertex_0], (point_3d_ptr)&the_object->vertices_local[vertex_1], (vector_3d_ptr)&u);

		// the vector v = vo->v2
		Make_Vector_3D((point_3d_ptr)&the_object->vertices_local[vertex_0], (point_3d_ptr)&the_object->vertices_local[vertex_2], (vector_3d_ptr)&v);

		Cross_Product_3D((vector_3d_ptr)&v, (vector_3d_ptr)&u, (vector_3d_ptr)&normal);
...
void Remove_Backfaces_And_Shade(object_ptr the_object)
{
	...
	for (curr_poly = 0; curr_poly < the_object->num_polys; curr_poly++) {
		...
		// the vector u = vo->v1
		Make_Vector_3D((point_3d_ptr)&the_object->vertices_world[vertex_0], (point_3d_ptr)&the_object->vertices_world[vertex_1], (vector_3d_ptr)&u);

		// the vector v = vo-v2
		Make_Vector_3D((point_3d_ptr)&the_object->vertices_world[vertex_0], (point_3d_ptr)&the_object->vertices_world[vertex_2], (vector_3d_ptr)&v);
		...
		if (dp > 0) {
			...
			if (the_object->polys[curr_poly].shading == FLAT_SHADING) {
				...
				if (dp > 0) {
					...
					the_object->polys[curr_poly].shade = the_object->polys[curr_poly].color - (int)intensity;

					// printf("\nintensity of polygon %d is %f",curr_poly,intensity);
				} else {
					the_object->polys[curr_poly].shade = the_object->polys[curr_poly].color - (int)ambient_light;
				}
			} else {
...
int Remove_Object(object_ptr the_object, int mode)
{
	...
	// compute x component
	x_bsphere = the_object->world_pos.x * global_view[0][0] + the_object->world_pos.y * global_view[1][0] + the_object->world_pos.z * global_view[2][0] + global_view[3][0];

	// compute y component
	y_bsphere = the_object->world_pos.x * global_view[0][1] + the_object->world_pos.y * global_view[1][1] + the_object->world_pos.z * global_view[2][1] + global_view[3][1];

	// compute z component
	z_bsphere = the_object->world_pos.x * global_view[0][2] + the_object->world_pos.y * global_view[1][2] + the_object->world_pos.z * global_view[2][2] + global_view[3][2];
...
void Generate_Poly_List(object_ptr the_object, int mode)
{
	...
	for (curr_poly = 0; curr_poly < the_object->num_polys; curr_poly++) {
		...
		if (the_object->polys[curr_poly].visible && !the_object->polys[curr_poly].clipped) {
			...
			for (curr_vertex = 0; curr_vertex < the_object->polys[curr_poly].num_points; curr_vertex++) {
				...
				// extract x,y and z
				world_poly_storage[num_polys_frame].vertex_list[curr_vertex].x = the_object->vertices_camera[vertex].x;
				world_poly_storage[num_polys_frame].vertex_list[curr_vertex].y = the_object->vertices_camera[vertex].y;
				world_poly_storage[num_polys_frame].vertex_list[curr_vertex].z = the_object->vertices_camera[vertex].z;
			}
...
void Draw_Line(int xo, int yo, int x1, int y1, unsigned char color, unsigned char *vb_start)
{
	...
	vb_start = vb_start + ((unsigned int)yo << 6) + ((unsigned int)yo << 8) + (unsigned int)xo;
...
void Draw_Object_Wire(object_ptr the_object)
{
	...
	for (curr_poly = 0; curr_poly < the_object->num_polys; curr_poly++) {
		...
		if (the_object->polys[curr_poly].visible == 0 || the_object->polys[curr_poly].clipped) {
			continue;
		}
		...
		for (curr_vertex = 0; curr_vertex < the_object->polys[curr_poly].num_points - 1; curr_vertex++) {
			...
			if (Clip_Line(&ix1, &iy1, &ix2, &iy2)) {
				Draw_Line((int)ix1, (int)iy1, (int)ix2, (int)iy2, (unsigned char)the_object->polys[curr_poly].color, RETRO.framebuffer);
			}
		}
...
void Draw_Object_Solid(object_ptr the_object)
{
	...
	for (curr_poly = 0; curr_poly < the_object->num_polys; curr_poly++) {
		...
		// draw triangle
		Draw_Triangle_2D((int)x1, (int)y1, (int)x2, (int)y2, (int)x3, (int)y3, the_object->polys[curr_poly].shade);
		...
		if (is_quad) {
			...
			// draw triangle
			Draw_Triangle_2D((int)x1, (int)y1, (int)x3, (int)y3, (int)x4, (int)y4, the_object->polys[curr_poly].shade);
		}
	}
}
...
void Draw_Poly_List(void)
{
	...
	for (curr_poly = 0; curr_poly < num_polys_frame; curr_poly++) {
		...
		// draw triangle
		Draw_Triangle_2D((int)x1, (int)y1, (int)x2, (int)y2, (int)x3, (int)y3, world_polys[curr_poly]->shade);
		...
		if (is_quad) {
			...
			// draw triangle
			Draw_Triangle_2D((int)x1, (int)y1, (int)x3, (int)y3, (int)x4, (int)y4, world_polys[curr_poly]->shade);
		}
	}
}
...
int Poly_Compare(facet **arg1, facet **arg2)
{
	...
	if (poly_1->num_points == 3) {
		...
		z1 = (float)0.33333 * (poly_1->vertex_list[0].z + poly_1->vertex_list[1].z + poly_1->vertex_list[2].z);
	} else {
	...
	if (poly_2->num_points == 3) {
		// compute average of 3 point polygon
		z2 = (float)0.33333 * (poly_2->vertex_list[0].z + poly_2->vertex_list[1].z + poly_2->vertex_list[2].z);
	} else {
		// compute average of 4 point polygon
...
void Print_Poly_List(void)
{
	...
	for (index = 0; index < num_polys_frame; index++) {
		if (world_polys[index]->num_points == 3) {
			z = (float)0.33333 * (world_polys[index]->vertex_list[0].z + world_polys[index]->vertex_list[1].z + world_polys[index]->vertex_list[2].z);
		} else {
			z = (float)0.25 * (world_polys[index]->vertex_list[0].z + world_polys[index]->vertex_list[1].z + world_polys[index]->vertex_list[2].z + world_polys[index]->vertex_list[3].z);
		}
...
int Load_Palette_Disk(const char *filename, RGB_palette_ptr the_palette)
{
	// this function loads a color palette from disk

	int index; // used for looping
	int red, green, blue; // used for to store temporary palette colors
	...
	for (index = 0; index <= 255; index++) {
		...
		the_palette->colors[index].red = red;
		the_palette->colors[index].green = green;
		the_palette->colors[index].blue = blue;
	}
