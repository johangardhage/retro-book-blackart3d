// Header file for chapter 18 - BLACK18.H
...
void Triangle_Line(unsigned char *dest_addr, unsigned int xs, unsigned int xe, int color)
{
	...
	// now blast the middle part of the line
...
void Triangle_QLine(unsigned char *dest_addr, unsigned int xs, unsigned int xe, unsigned int color)
{
	...
	memset((char *)dest_addr, (unsigned char)color, xe - xs + 1);
}
...
void Create_World_To_Camera(void)
{
	...
	case 1: // translation and X axis
	{
		...
		global_view[3][1] = (-view_point.y * cos_look[view_angle.ang_y] - view_point.z * sin_look[view_angle.ang_y]);
		global_view[3][2] = (view_point.y * sin_look[view_angle.ang_y] - view_point.z * cos_look[view_angle.ang_y]);
	} break;
	...
	case 2: // translation and Y axis
	{
		...
		// now copy last row into global_view
		global_view[3][0] = (-view_point.x * cos_look[view_angle.ang_y] + view_point.z * sin_look[view_angle.ang_y]);
		global_view[3][1] = (-view_point.y);
		global_view[3][2] = (-view_point.x * sin_look[view_angle.ang_y] - view_point.z * cos_look[view_angle.ang_y]);
	} break;
	...
	case 4: // translation and Z
	{
		...
		// now copy last row into global_view
		global_view[3][0] = (-view_point.x * cos_look[view_angle.ang_z] - view_point.y * sin_look[view_angle.ang_z]);
		global_view[3][1] = (view_point.x * sin_look[view_angle.ang_z] - view_point.y * cos_look[view_angle.ang_z]);
...
void Clip_Object_3D(object_ptr the_object, int mode)
{
	...
	float x1, y1, z1,
		...
		fov_height; // used to speed up computations, since it's constant for any frame
...
int Save_Palette_Disk(const char *filename, RGB_palette_ptr the_palette)
{
...
{
	// this function fills in the double buffer

	memset((char *)RETRO.framebuffer, (unsigned char)icolor, double_buffer_size);
}
...
int Clip_Line(int *x1, int *y1, int *x2, int *y2)
{
	...
	if (point_1 == 0 && point_2 == 0) {
		...
		// if we got here we have the special case where the line cuts into and out of the clipping region
		clip_always = 1;
	}
	...
	if ((point_1 == 1) || (point_1 == 0 && point_2 == 0)) {
		...
		// now we know where the line passed thru compute which edge is the proper intersection
	...
	for (curr_poly = 0; curr_poly < the_object->num_polys; curr_poly++) {
		...
		for (curr_vertex = 0; curr_vertex < the_object->polys[curr_poly].num_points - 1; curr_vertex++) {
			...
			if (Clip_Line(&ix1, &iy1, &ix2, &iy2)) {
				Draw_Line((int)ix1, (int)iy1, (int)ix2, (int)iy2, (unsigned char)color, RETRO.framebuffer);
			}
		}
