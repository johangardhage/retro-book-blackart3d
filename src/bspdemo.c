...
#define MAX_LINES        64   // this is the maximum number of lines in the demo, these lines will later become 3-D walls
...
#define POINTER_END    1  // the modified cross hair that indicates the end of a line is being drawn
#define POINTER_DEL    2  // this is a modified cross hair that indicates a line is to be deleted
...
line_2d lines[MAX_LINES]; // this holds the lines in the system, we could use a linked list, but let's not make this harder than it already is!
...
int mouse_x,          // mouse position
mouse_y,
buttons,          // state of buttons
button_down = 0,
sel,
action = ACTION_STARTING_LINE; // this is used to track the current action the user is in the middle of
...
void Convert_Lines_To_Walls(void)
{
	...
	for (index = 0; index < total_lines; index++) {
		...
		Print_String_DB((lines[index].sx + lines[index].ex) / 2, (lines[index].sy + lines[index].ey) / 2, 10, buffer, 1);
		...
		Make_Vector_3D((point_3d_ptr)&temp_wall->wall_world[0], (point_3d_ptr)&temp_wall->wall_world[1], (vector_3d_ptr)&u);
		Make_Vector_3D((point_3d_ptr)&temp_wall->wall_world[0], (point_3d_ptr)&temp_wall->wall_world[3], (vector_3d_ptr)&v);

		// use cross product to compute normal
		Cross_Product_3D((vector_3d_ptr)&u, (vector_3d_ptr)&v, (vector_3d_ptr)&temp_wall->normal);
...
int Delete_Line(int x, int y)
{
	...
	for (curr_line = 0; curr_line < total_lines; curr_line++) {
		...
		if (x >= min_x && x <= max_x && y >= min_y && y <= max_y) {
			...
			test_error = (float)(100 * abs((length_1 + length_2) - length_line)) / (float)length_line;
	...
	if (best_line != -1) {
		...
		if (total_lines == 1) { // a single line
			total_lines = 0;
		} else if (best_line == total_lines - 1) {  // the line to delete is the last in array
			total_lines--;
		} else {
		...
		PCX_Copy_To_Buffer((pcx_picture_ptr)&interface_pcx, RETRO.framebuffer);
...
int Button_Pressed(gadget_ptr gadgets, int num_gadgets, int pos_x, int pos_y)
{
	...
	for (curr_gadget = 0; curr_gadget < num_gadgets; curr_gadget++) {
		...
		// gadgets
		if (pos_x > gadgets[curr_gadget].x && pos_x < (gadgets[curr_gadget].x + GADGET_WIDTH) && pos_y >  gadgets[curr_gadget].y && pos_y < (gadgets[curr_gadget].y + GADGET_HEIGHT)) {
			// click the button
			controls_spr.x = gadgets[curr_gadget].x;
			controls_spr.y = gadgets[curr_gadget].y;

			controls_spr.curr_frame = gadgets[curr_gadget].id + 7;
			Sprite_Draw_Clip((sprite_ptr)&controls_spr, RETRO.framebuffer, 1);
			Sprite_Draw_Clip((sprite_ptr)&pointer_spr, RETRO.framebuffer, 1);
...
void DEMO_Render2(double deltatime)
{
	...
	// erase the pointer
	Sprite_Erase_Clip((sprite_ptr)&pointer_spr, RETRO.framebuffer);

	// map the mouse position to the screen and assign it to pointer
	pointer_spr.x = mouse_x - 4;
	pointer_spr.y = mouse_y - 4;
	...
	if (leftMouseButtonClicked) {
		...
		if (mouse_x >= BSP_MIN_X && mouse_x <= BSP_MAX_X &&
			...
			if (action == ACTION_STARTING_LINE) {
				...
				lines[total_lines].sx = mouse_x;
				lines[total_lines].sy = mouse_y;
				...
				lines[total_lines].ex = mouse_x;
				lines[total_lines].ey = mouse_y;

				// set point type to ending point
				action = ACTION_STARTING_LINE;

				// draw a line between points
				Draw_Line(lines[total_lines].sx, lines[total_lines].sy, lines[total_lines].ex, lines[total_lines].ey, 13, RETRO.framebuffer);
	...
	// scan under pointer
	Sprite_Under_Clip((sprite_ptr)&pointer_spr, RETRO.framebuffer);

	// draw the pointer
	Sprite_Draw_Clip((sprite_ptr)&pointer_spr, RETRO.framebuffer, 1);

	// draw the coordinates
	sprintf(buffer, "(%d,%d) ", mouse_x + SCREEN_TO_WORLD_X, -(mouse_y + SCREEN_TO_WORLD_Z));
...
void DEMO_Initialize(void)
{
	...
	PCX_Load("assets/bspbutt.pcx", (pcx_picture_ptr)&image_pcx, 1);
	...
	for (index = 0; index < 14; index++) {
		PCX_Get_Sprite((pcx_picture_ptr)&image_pcx, (sprite_ptr)&controls_spr, index, index / 7, index % 7);
	}
	...
	PCX_Init((pcx_picture_ptr)&image_pcx);
	PCX_Load("assets/bsppoint.pcx", (pcx_picture_ptr)&image_pcx, 1);
	...
	for (index = 0; index < 4; index++) {
		PCX_Get_Sprite((pcx_picture_ptr)&image_pcx, (sprite_ptr)&pointer_spr, index, index, 0);
	}
	...
	PCX_Load("assets/bspint.pcx", (pcx_picture_ptr)&interface_pcx, 1);
