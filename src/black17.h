// Header file for chapter 17 - BLACK17.H
...
void World_To_Camera_Object(object_ptr the_object)
{
	...
	case 0:   // T-1
	{
		for (index = 0; index <= the_object->num_vertices; index++) {
			...
			// x component
			the_object->vertices_camera[index].x = the_object->vertices_world[index].x + global_view[3][0];

			// y component
			the_object->vertices_camera[index].y = the_object->vertices_world[index].y + global_view[3][1];

			// z component
			the_object->vertices_camera[index].z = the_object->vertices_world[index].z + global_view[3][2];
		}
	} break;
